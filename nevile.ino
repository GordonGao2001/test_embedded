...
void loop() {
  ...
  if(ultrasonic.read()<=5){
    ...
    if(sensor_zeros>=6){//for 20ms there is no more sensing
        activateMotor(0, 0);
        activateMotor(1, 0);
        while(true){
    ...
    else if(((sensors[2]+sensors[1])/2) >= sensors[0] && ((sensors[2]+sensors[1])/2) >= sensors[3] && ((sensors[2]+sensors[1])/2) >= sensors[4])
    {
      sensor_zeros=0;
      activateMotor(0, 0.2);
      activateMotor(1, 0.2);
    }
    else if(sensors[3] >= sensors[0] && sensors[3] >= ((sensors[2]+sensors[1])/2) && sensors[3] >= sensors[4])
    {
      sensor_zeros=0;
      activateMotor(1, 0.2);
      activateMotor(0, -0.05);
    }
    else if(sensors[0] >= ((sensors[2]+sensors[1])/2) && sensors[0] >= sensors[3] && sensors[0] >= sensors[4])
    {
      sensor_zeros=0;
      activateMotor(1, 0.2);
      activateMotor(0, -0.05);
    }
    else if(sensors[4] >= sensors[3] && sensors[4] >= ((sensors[2]+sensors[1])/2) && sensors[4] >= sensors[0])
    {
      sensor_zeros=0;
      activateMotor(0, 0.2);
      activateMotor(1,-0.05);
    }
  }
}
